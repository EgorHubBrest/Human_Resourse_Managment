...
class UserTests(APITestCase):
    ...
    def test_user_negative(self):
        ...
        response_reg = self.client.post(url, data, format='json')
        self.assertEqual(response_reg.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        response_reg = self.client.post(url, data, format='json')
        response_content = json.loads(response_reg.content.decode('utf-8'))

        url = reverse('department_app:change-password')
        data = {
            "old_password": "q52wrw75765reasdzxc",
            "new_password": "Newqwrw75765reasdzxc"
        }
        response_user_put = self.client.put(
            url, data, HTTP_AUTHORIZATION='Token {}'.format(response_content['token']))
        self.assertEqual(response_user_put.status_code,
                         status.HTTP_400_BAD_REQUEST)
